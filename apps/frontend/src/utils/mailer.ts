...
export const fetchSportsPlaylist = async (name: string, email: string, sportName: string, skillLevel: string, keywords: string) => {
    try {
        ...
        const response = await fetch(`${BACKEND_URL}/api/v1/training/fetch-playlist`, {
            ...
            body: JSON.stringify({ name, email, sportName, skillLevel, keywords }),
        });
