...
// import { useAuth } from "../../context/useAuth"; // Your auth context
import { useWatchedStories } from "../../context/useWatchedStories";

export const StoryList: React.FC = () => {
  // const { user } = useAuth();
  // const userId = user?.id;

  const { viewedStoryImages: watchedStories, markImageAsViewed } = useWatchedStories();
  ...
  // Use watchedStories from context instead of local state and localStorage
  // watchedStories: { [storyId: number]: number[] } 

  const isStoryFullyViewed = (story: StoryType) => {
    const viewedImagesForStory = watchedStories[story.id] || [];
    ...
    <div className="flex flex-col p-2 relative">
          ...
          >
            ...
            {sortedStories.length > 0 ? (
              sortedStories.map((storyItem) => {
                const allImageIds = storyItem.Storyimages.map(img => img.id);
                const viewedImagesForStory = watchedStories[storyItem.id] || [];
                const isViewed = allImageIds.every(id => viewedImagesForStory.includes(id));
      ...
      {openStory && (
        <StoryView
          ...
          viewedImages={watchedStories[openStory.id] || []}
...
// import { StoryType } from "./types";
// import StoryView from "./StoryView";
// import { useAuth } from "../../context/useAuth"; // Make sure you have this context
...
//   // Store viewed story images keyed by storyId: imageId[]
...
//   const getViewedImagesKey = (userId: number) => `viewedStoryImages_user_${userId}`;

//   // Load viewedStoryImages from localStorage scoped to current user
//   useEffect(() => {
//     if (typeof userId !== "number") return;
//     const saved = localStorage.getItem(getViewedImagesKey(userId));
//     if (saved) {
//       try {
//         setViewedStoryImages(JSON.parse(saved));
//       } catch {
//         localStorage.removeItem(getViewedImagesKey(userId));
//       }
//     } else {
...
//   // Save viewedStoryImages on every change, scoped to user
//   useEffect(() => {
//     if (typeof userId !== "number") return;
//     localStorage.setItem(getViewedImagesKey(userId), JSON.stringify(viewedStoryImages));
//   }, [viewedStoryImages, userId]);
...
//         return { ...prev, [storyId]: updatedImages };
//       }
//       return prev;
//     });
//   };
...
//               sortedStories.map((storyItem) => {
//                 const allImageIds = storyItem.Storyimages.map(img => img.id);
//                 const viewedImagesForStory = viewedStoryImages[storyItem.id] || [];
//                 const isViewed = allImageIds.every(id => viewedImagesForStory.includes(id));
...
//           story={openStory}
//           onClose={closeStoryView}
//           viewedImages={viewedStoryImages[openStory.id] || []}
//           markImageAsViewed={markImageAsViewed}
//         />
//       )}
//     </div>
