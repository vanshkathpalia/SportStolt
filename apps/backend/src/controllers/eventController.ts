...
export const createEventHandler = async (c: Context) => {
  try {
    ...
    const event = await eventService.createEvent(c.env.DATABASE_URL, body, Number(userId));
    return c.json({ id: event.id, authorId: event.authorId });
  } catch (error: unknown) {
    if (error instanceof Error) {
      if (error.message === "Start time must be in the future") {
        return c.json({ message: "Start time must be in the future" }, 400);
      }
...
export const getBulkEventsHandler = async (c: Context) => {
  try {
    const events = await eventService.getBulkEvents(c.env.DATABASE_URL);
...
export const registerUserHandler = async (c: Context) => {
  try {
    ...
    const registration = await eventService.registerUser(c.env.DATABASE_URL, Number(userId), eventId);
    return c.json({ message: "Successfully registered", registration });
  } catch (error : unknown) {
    if (error instanceof Error) {
      if (error.message === "Already registered") {
        return c.json({ message: "Already registered for this eventd" }, 404);
      }
...
export const getRegistrationsHandler = async (c: Context) => {
  try {
    const eventId = Number(c.req.param('id'));
    const registrations = await eventService.getRegistrations(c.env.DATABASE_URL, eventId);
...
export const deleteEventHandler = async (c: Context) => {
  try {
    ...
    const result = await eventService.deleteEvent(c.env.DATABASE_URL, eventId, Number(userId));
...
// import { Context } from "hono";
// import { createEventInput } from "../schema/eventSchema";
// import { sendEventNotification, EventInterface } from "./notificationController";
// import * as eventService from "../services/eventService";
...
//     const event = await eventService.createEvent(c.env.DATABASE_URL, body, Number(userId));
//     await sendEventNotification(null, event as EventInterface, Number(userId));
...
//     if (!userId) {
//       return c.json({ message: "User not authenticated" }, 403);
//     }
...
// //     const eve-nts = await prisma.event.findMany({
// //       where: {
// //         isArchived: false,
// //       },
// //       select: {
