...
export async function calculatePointsForStory(
  story: Story & { Storyimages: Storyimages[], attendees: any[], author: User }
): Promise<number> {
  
  // Don’t reward unpaid users
  if (!story.author.hasPaid) return 0;

  let points = 0;

  // Points from image verifications
  const totalVerifications = story.Storyimages.reduce(
    (acc, img) => acc + img.verificationCount,
    0
  );
  points += totalVerifications * 2;

  // Points from attendees
  points += story.attendees.length;

  // Optional: Cap max points if needed
